...
- `AuthService` guarda, junto do token, o objeto `consent: { required, termsVersion, comms }` vindo da resposta de `/partner/session` (spec irmã §3.1). Validação de shape: campo **opcional** na validação da resposta (`auth.service.ts:87-89`) — servidor antigo sem o campo ⇒ tratar como `required: false` (widget novo continua funcionando contra servidor antigo).
...
- Título **"Antes de começar"** (28px/34px, weight 500, letter-spacing -0.02em) · descrição **"Para usar o EvidenceOne, precisamos do seu aceite."**
...
- **Checkbox 2 (opcional)** — sem card: "Quero receber novidades e melhorias do EvidenceOne em primeira mão". **Prefill:** primeiro acesso ⇒ desmarcado; re-consent (nova versão dos Termos) ⇒ pré-marcado conforme `consent.comms` do servidor. Termos **sempre** começa desmarcado.
...
| Comms no re-consent | Prefill com `consent.comms` do servidor | Sempre desmarcado | Evita revogação silenciosa de comms ao re-aceitar Termos novos; "começa desmarcado" vale para o 1º acesso |
