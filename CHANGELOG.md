...
- **Maintenance screen** (`eo-maintenance`). "Estamos em manutenção" replaces the
  drawer body and the composer while `eo-chat.maintenance` is set, and
  "Tentar novamente" asks for a re-check (`eoMaintenanceRetry`). Internal for
  now: the root component starts driving it once the server exposes the
  maintenance state. The public `<evidenceone-chat>` API is unchanged.
...
- **Page translators no longer brick the widget with "erro de integridade".** In
  production, Chrome auto-translate rewrote the trigger label as the widget
  mounted (translators rewrite dynamically-inserted nodes immediately, open
  shadow roots included), so the mount-time brand-hash check read altered text
  and failed closed. The brand surfaces — trigger button/label and the header
  logo + wordmark — are now marked `translate="no"` + `.notranslate`, which
  Chrome, Edge, and the Google Translate widget honor. All other widget copy
  remains translatable. Translators/extensions that ignore the attribute still
  trip the lock — from the inside that is indistinguishable from tampering,
  and genuine tampering must keep failing closed
  (`test/translate-simulation.html` reproduces both cases).
- Integrity-failure console message now includes the rendered label and a
  page-translator hint, so partner support can triage without a debug build.
